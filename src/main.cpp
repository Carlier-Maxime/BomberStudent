...
int main() {
    ...
    try {
        SocketAddress addr1 = SocketAddress("127.0.0.1",42069,Protocol::IPV4);
        SocketAddress addr2 = SocketAddress("127.0.0.1",42469,Protocol::IPV4);
        Socket socket1=Socket(addr1);
        Socket socket2=Socket(addr2);
        socket1.send("hello world !\n", addr2);
        std::cout << socket2.receive(addr1);
